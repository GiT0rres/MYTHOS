...
class MythologyRepository(
    // FirebaseFirestore.getInstance() lança IllegalStateException se o FirebaseApp nunca foi
    // inicializado — é o que acontece no sandbox do Compose Preview (Layoutlib). Guardando com
    // try/catch, o repositório continua instanciável nesse ambiente e todos os métodos abaixo
    // caem automaticamente para o acervo local (LOCAL_DEITIES / LOCAL_COMPARISONS).
    ...
    suspend fun seedIfEmpty() {
        ...
        val deities = firestore.collection("deities").get().await()
        if (deities.isEmpty) {
            LOCAL_DEITIES.forEach { deity ->
                firestore.collection("deities").document(deity.id).set(deity).await()
            }
        }
        val comparisons = firestore.collection("comparisons").get().await()
        if (comparisons.isEmpty) {
            LOCAL_COMPARISONS.forEach { comparison ->
                firestore.collection("comparisons").document(comparison.id).set(comparison).await()
            }
        }
    }

    suspend fun getDeities(): List<Deity> {
        val firestore = db ?: return LOCAL_DEITIES
        val snapshot = firestore.collection("deities").get().await()
    ...
    suspend fun getCuriosity(firstId: String, secondId: String): String {
        val fallback = "Culturas diferentes representaram forças da natureza em esculturas e " +
                "afrescos, criando figuras divinas com atributos muito parecidos."
        val firestore = db ?: run {
            return LOCAL_COMPARISONS.firstOrNull {
                (it.firstId == firstId && it.secondId == secondId) ||
                        (it.firstId == secondId && it.secondId == firstId)
            }?.curiosity ?: fallback
        }
        val snapshot = firestore.collection("comparisons").get().await()
        ...
        return found?.curiosity ?: fallback
    }
